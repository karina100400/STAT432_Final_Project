...
# Learning Rate 0.1
Glioma_GBM1 <- gbm(Grade ~., data=Glioma_train_boosting, distribution="bernoulli", n.trees=1000, interaction.depth = 2, shrinkage=0.1, cv.folds = 5)
#finding optimal number of trees
best_iter1 <- gbm.perf(Glioma_GBM1, method = "cv")
#getting class labels
pred <- predict(Glioma_GBM1, newdata = Glioma_test, n.trees = best_iter1, type="response")
#converting reponse variable back into string for conf matrix
Glioma_GBM1_predicted <- ifelse(pred > 0.5, 1, 0)
...
set.seed(7)
# Learning Rate 0.01
Glioma_GBM2 <- gbm(Grade ~., data=Glioma_train_boosting, distribution="bernoulli", n.trees=3000, interaction.depth = 2, shrinkage=0.01, cv.folds = 5)
best_iter2 <- gbm.perf(Glioma_GBM2, method = "cv")

pred <- predict(Glioma_GBM2, newdata = Glioma_test, n.trees = best_iter2, type="response")
Glioma_GBM2_predicted <- ifelse(pred > 0.5, 1, 0)
...
set.seed(7)
# Learning Rate 0.001
Glioma_GBM3 <- gbm(Grade ~., data=Glioma_train_boosting, distribution="bernoulli", n.trees=12000, shrinkage=0.001, cv.folds = 5)
best_iter3 <- gbm.perf(Glioma_GBM3, method = "cv")

pred <- predict(Glioma_GBM3, newdata = Glioma_test, n.trees = best_iter3, type="response")
Glioma_GBM3_predicted <- ifelse(pred > 0.5, 1, 0)
Glioma_GBM3_predicted <- ifelse(Glioma_GBM3_predicted == 1, "GBM", "LGG")

Glioma_GBM3_conf_matrix <- table(Predicted = Glioma_GBM3_predicted, Actual = Glioma_test$Grade)
Glioma_GBM3_conf_matrix

Glioma_GBM3_test_error <- 1-(sum(diag(Glioma_GBM3_conf_matrix))/sum(Glioma_GBM3_conf_matrix))
Glioma_GBM3_test_error
```
There was a negligible difference in the three different GBM models.
```{r}
#use caret tune grid
```
