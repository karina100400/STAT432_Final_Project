...
pred <- predict(Glioma_Ada, newdata = Glioma_test, n.trees = best_iter2, type="response")
Glioma_Ada_predicted <- ifelse(pred > 0.5, 1, 0)
...
```
Adaboost performed very similarly to GBM.
...
Glioma_SVM_linear_pred <- predict(Glioma_SVM_linear, Glioma_test) #returns labels directly
...
Glioma_SVM_radial_pred <- predict(Glioma_SVM_radial, Glioma_test) #returns labels directly
...
```
```{r}
tune_poly=tune(svm, Grade~., data=Glioma_train, kernel ="polynomial", degree=2,
ranges =list(cost=c(0.001 , 0.01, 0.1, 1,5,10,100))
)
summary(tune.out)
```
...
Glioma_SVM_poly_pred <- predict(Glioma_SVM_poly, Glioma_test) #returns labels directly
...
```
All three SVM models performed similarly.
